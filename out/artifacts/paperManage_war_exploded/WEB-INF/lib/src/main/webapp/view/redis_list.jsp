...
<body onload="load()">
	<div class="x-nav">
		<span class="layui-breadcrumb"> <a>职工管理</a> <a> <cite>职工基本信息管理</cite></a>
			...
			href="userController/toUserList.do" title="刷新"> <i
	...
	<div class="x-body">
		<div class="layui-row">
			...
			<!-- 这个地方等于就是改造后台查询方法 -->
			<form class="layui-form layui-col-md12 x-so" method="post" action="userController/toUserList.do">
				<span>性别查询：</span>
...
<script type="text/javascript">
	//文档加载事件:在加载这个jsp就会自动触发的函数
	$(function() {
		//利用ajax去后台查询出指定Cookie
		$.ajax({  //ajax封装的信息都是以键值对的形式 
				//设置请求方式
				type:"post",
				//请求服务器(后台)的地址
				url:"userController/allUser.ajax",
				//需要传入到服务器的数据  传值方式也是以键值对的形式
				data:{},
				//设置返回值类型 text:表示返回的是字符串 json:表示集合
				dataType:"text",
				//成功的回调函数  info:返回的内容 
				success:function(info){
					//alert(info);
